...
- 1st: standard simple starting program: print: "hello world"
...
#### Note: for some models this code may mostly work, but the details of the input and output may need to be changed slightly (also depending on how you want/need the output formatted) e.g. if you need the output to be in square brackets or not.
```
import json
from joblib import dump, load
import boto3
...
#### another example for logistic regression models with one input
```
import json
from joblib import dump, load
import boto3
...
#### Note: You most likely will NOT want to or need to simply use the requirements list generated by 'freeze' or a similar item. e.g. AWS pre-installs packages and double-installing what is already there can break everything. And in general, if all you need is scikit-learn, then you should just say 'scikit-learn,' not listing all the dependencies automatically installed when you install scikit-learn.

- (ONLY IF YOU REALLY NEED THIS) Run in a Terminal
```
